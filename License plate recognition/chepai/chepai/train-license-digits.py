...
def save_and_convert_to_tflite(model_dir, tflite_model_path):
    graph = tf.Graph()
    with graph.as_default():
        with tf.Session() as sess:
            # Restore the graph
            saver = tf.train.import_meta_graph("%smodel.ckpt.meta" % (model_dir))
            ...
            input_graph_def = tf.graph_util.remove_training_nodes(sess.graph_def)
            ...
            # Remove the frozen graph file
            os.remove('frozen_graph.pb')


def convert_and_save_model(sess, model_path):
    # 获取计算图
    ...
    # 将模型保存为.tflite文件
    tflite_model_file = "./Model/license_digits.tflite"
    with open(tflite_model_file, "wb") as f:
        f.write(tflite_model)

    print("Model converted and saved to", tflite_model_file)


if __name__ == '__main__' and sys.argv[1] == 'packaging':
    tflite_model_path = './Model/model.tflite'

    if not os.path.exists('./Model'):
        os.makedirs('./Model')

    save_and_convert_to_tflite(SAVER_DIR, tflite_model_path)
    print("Model has been successfully converted to TFLite and saved to", tflite_model_path)
    # saver = tf.train.import_meta_graph("%smodel.ckpt.meta" % (SAVER_DIR))
    # with tf.Session() as sess:
    #     model_file = tf.train.latest_checkpoint(SAVER_DIR)
    #     saver.restore(sess, model_file)
    ...
    #     model_path = "./Model/license_digits_saved_model"
    #     convert_and_save_model(sess, model_path)